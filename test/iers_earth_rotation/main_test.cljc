(ns iers-earth-rotation.main-test
  "Contract + behavioral test for the iers_earth_rotation-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "iers_earth_rotation-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'iers-earth-rotation.main-test)]
