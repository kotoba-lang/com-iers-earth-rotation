(ns iers-earth-rotation.main
  "Kotodama WASM entrypoint for the Iers_earth_rotation clean-room actor (L4) — Clojure port.
  ...
  facts (`iers_earth_rotation.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "iers_earth_rotation")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"iers_earth_rotation.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "iers_earth_rotation-compat" :tier tier :entities entities} 200])
...
(defn start! [] :iers_earth_rotation-compat/ready)
